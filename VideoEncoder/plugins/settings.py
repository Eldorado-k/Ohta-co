...
async def settings_viewer(bot: Client, event: Message):
    ...
    # Reply Text
    vset = f'''<b>Paramètres de l'Encodeur :</b>
...
Réglage : {'Animation' if ((await db.get_tune(user_id)) is True) else 'Film'}
Préréglage : {pre}
...
<b>©️ Paramètres du Filigrane</b>
Métadonnées : {metadata}
Vidéo {'☑️' if ((await db.get_watermark(user_id)) is True) else ''}

<b>🔊 Paramètres Audio</b>
Codec : {audio}
Taux d'échantillonnage : {sample}
