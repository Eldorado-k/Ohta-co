...
async def docupload(client, message):
    ...
    if not file:
        await message.reply('vous avez oublié de mentionner le chemin !')
        return
    text = f'Envoi de {html.escape(file)}...'
    ...
    try:
        ...
        await reply.edit('Erreur lors de l\'envoi ! {}'.format(e))
    else:
        await reply.delete()
...
async def videoupload(client, message):
    ...
    file = os.path.expanduser(' '.join(message.command[1:]))
    if not file:
        await message.reply_text('vous avez oublié de mentionner le chemin du fichier !')
        return
    filename = os.path.basename(file)
...
async def driveupload(client, message):
    ...
    if not new_file:
        await message.reply_text('vous avez oublié de mentionner le chemin !')
        return
    text = f'Envoi de {html.escape(new_file)}...'
    reply = await message.reply_text(text)
    try:
        u = Uploader()
        await u.upload_to_drive(new_file, message, reply)
    except:
        await reply.edit('Erreur lors de l\'envoi !')
    else:
        await reply.delete()
...
async def logsup(client, message):
    ...
    try:
        ...
        await message.reply('Envoi annulé !')
    except Exception as e:
