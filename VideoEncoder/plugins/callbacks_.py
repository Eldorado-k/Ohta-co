...
reply_markup = InlineKeyboardMarkup(
    [
        ...
        [
            ...
            InlineKeyboardButton("📖 Aide", url="https://t.me/BTZF_CHAT")
        ]
    ]
)
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    ...
    # Reset Settings
        ...
        try:
            ...
            await cb.answer("Your settings have been reset successfully!")

            await cb.message.edit(
                text="Your settings have been reset to the default values <a href='https://telegra.ph/file/11379aba315ba245ebc7b.jpg'>!</a>.",
                reply_markup=reply_markup
            )

        except Exception as e:
            await cb.answer(f"Error: {e}")
            await cb.message.edit(text=f"Failed to reset settings: {e}")
    ...
    # Watermark
        ...
        await cb.answer("Yo. ce bouton  marche pas🙃\n\nClique sur ceux d'en bas.", show_alert=True)
    ...
    # Audio Channel
        ...
        if c == 'source':
            ...
            await cb.answer("La version 7.1 est uniquement pour Blu-ray.", show_alert=True)
    ...
    # Video Bits
        ...
        if await db.get_hevc(cb.from_user.id):
            ...
            if b:
                ...
                await cb.answer("H264 ne prend pas en charge les 10 bits dans ce bot.", show_alert=True)
        await VideoSettings(cb.message, user_id=cb.from_user.id)
    ...
    # HEVC
        ...
        if await db.get_hevc(cb.from_user.id):
            ...
            await cb.answer("H265 nécessite plus de temps pour l'encodage vidéo", show_alert=True)
        await VideoSettings(cb.message, user_id=cb.from_user.id)
    ...
    # Reframe
        ...
        if rf == '4':
            ...
            await cb.answer("Le Reframe 16 n'est peut-être pas supporté", show_alert=True)
        elif rf == '16':
    ...
    # Aspect
        ...
        if await db.get_aspect(cb.from_user.id):
            ...
            await cb.answer("Cela aidera à forcer la vidéo en 16:9", show_alert=True)
        await VideoSettings(cb.message, user_id=cb.from_user.id)
    ...
    # Cancel
        ...
        with open(status, 'r+') as f:
            ...
            try:
                await cb.message.edit_text("🚦🚦 Processus annulé 🚦🚦")
                chat_id = log
                ...
                now = f"\n{ist} (GMT+05:30)`\n`{bst} (GMT+06:00)"
                await bot.send_message(chat_id, f"**Dernier processus annulé, le bot est maintenant libre !!** \n\nProcessus terminé à `{now}`", parse_mode="markdown")
            except:
                pass
