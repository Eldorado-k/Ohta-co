...
async def encode(filepath, message, msg):
    ...
    if os.path.isfile(output_filepath):
        LOGGER.warning(f'"{output_filepath}": Le fichier existe déjà!')
    else:
...
async def handle_progress(proc, msg, message, filepath):
    ...
    while proc.returncode == None:
        ...
        with open(download_dir + 'process.txt', 'r+') as file:
            ...
            progress_str = "<b>Encodage du vidéo:</b> {0}%\n{1}{2}".format(
                round(percentage, 2),
                ''.join(['█' for i in range(
            ...
            try:
                await msg.edit(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton('Annuler', callback_data='cancel'), InlineKeyboardButton(
                                    'Statistiques', callback_data='stats')
                            ]
                        ]
                    )
                )
