...
output = InlineKeyboardMarkup([
    [
        InlineKeyboardButton("👨‍💻 Développeur", url="https://t.me/Kingcey"),
...
start_but = InlineKeyboardMarkup([
    ...
    [
        InlineKeyboardButton("👨‍💻 Développeur", url="https://t.me/ZFlixTeamBot"),
        InlineKeyboardButton("Dev", url="https://t.me/ZFlixTeamBot")
    ]
])
...
async def handle_encode(filepath, message, msg):
    if await db.get_hardsub(message.from_user.id):
        ...
        if not subs:
            await msg.edit("Quelque chose s'est mal passé pendant l'extraction des sous-titres!")
            return
    new_file = await encode(filepath, message, msg)
    if new_file:
        await msg.edit("<code>Fichier encodé, récupération des métadonnées...</code>")
        link = await upload_worker(new_file, message, msg)
        await msg.edit('Fichier encodé avec succès! Lien: {}'.format(link))
    else:
        await message.reply("<code>Quelque chose s'est mal passé pendant l'encodage de votre fichier.</code>")
    return link
...
async def get_zip_folder(orig_path: str):
    if orig_path.endswith(".tar.bz2"):
        ...
        raise IndexError("Format de fichier non supporté pour l'extraction!")
